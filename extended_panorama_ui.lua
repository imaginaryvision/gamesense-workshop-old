...
local friends_list_js = panorama.loadstring([[
...
local accept_js = panorama.loadstring([[
	var debug = false
...
local cancel_js = panorama.loadstring([[
...
looking_to_play_js.create(looking_to_play_layout)
friends_list_js.create(friends_list_layout)
accept_js.create(accept_layout)
cancel_js.create()

client.set_event_callback("shutdown", function()
	looking_to_play_js.destroy()
	friends_list_js.destroy()
	accept_js.destroy()
	cancel_js.destroy()
end)

local steamid_pattern = "^765" .. string.rep("%d", 14) .. "$"
local last_reload = 0
local invite_btn_job_active = false
client.set_event_callback("paint_ui", function()
	if globals.realtime() > last_reload+15 then
		local friends = {}
		local contents = readfile("friends.txt")

		if contents ~= nil then
			for steamid in contents:gmatch("[^\r\n]+") do
				steamid = steamid:gsub(" ", "")
				if steamid:match(steamid_pattern) then
					table.insert(friends, steamid)
				end
			end
		end
		friends_list_js.set_custom_friends(friends)
		last_reload = globals.realtime()
	end

	if invite_btn_job_active then
		local res = friends_list_js.invite_btn_job()
		if not res then
			invite_btn_job_active = false
		end
	end

	if get_clipboard_text ~= nil and friends_list_js.invite_btn_pressed() then
		local text = get_clipboard_text()

		if type(text) == "string" then
			friends_list_js.invite_btn_callback(text)
			invite_btn_job_active = true
		end
	end
end)